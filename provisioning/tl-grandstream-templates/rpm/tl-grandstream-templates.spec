BuildArch:     noarch
Name:          tl-grandstream-templates-%{type}
Version:       %{version}
Release:       %{release}%{?dist}
License:       GPL 
Group:         Utilities/System
Summary:       Grandstream Provisioning Templates for Thirdlane MTE PBX
...
Requires:      grandstream-firmwareGXP2130 >= 1.0.9.69
Requires:      grandstream-firmwareGXP2135 >= 1.0.9.69
Requires:      grandstream-firmwareGXP2140 >= 1.0.9.69
Requires:      grandstream-firmwareGXP2160 >= 1.0.9.69
Requires:      grandstream-firmwareGXP2170 >= 1.0.9.69
...
install -m 644 gs_pval_dict.txt %{buildroot}/etc/asterisk/provisioning/
install -m 644 gs.txt %{buildroot}/etc/asterisk/provisioning/models.d/
install -m 755 generate_compdir_gs.py %{buildroot}/usr/local/sbin/
install -m 755 gs-convert-to-xml.py %{buildroot}/usr/local/sbin/

%files
%defattr(644,root,root)
/etc/asterisk/provisioning/gs_gxp21xx_*.cfg
/etc/asterisk/provisioning/gs_pval_dict.txt
/etc/asterisk/provisioning/models.d/gs.txt
%dir %attr(0751, root, root) /home/PlcmSpIp
%attr(0755, root, root) /usr/local/sbin/generate_compdir_gs.py
%attr(0755, root, root) /usr/local/sbin/gs-convert-to-xml.py
...
if notifyConfig.has_section('gs-check-cfg'):
...
c = db.cursor()
try:
   if c.execute ("select (1) from provisioning_maps where name = 'grandstream' and category = 'tz' limit 1"):
      ...
      VALUES (%%s, %%s, %%s, %%s, %%s)""",
         [
         ('grandstream', 'tz', 'Grandstream', 'default', 'EST5EDT'),
         ('grandstream', 'tz', 'Grandstream', 'EST5EDT', 'EST5EDT'),
         ('grandstream', 'tz', 'Grandstream', 'CST6CDT', 'CST6CDT'),
         ('grandstream', 'tz', 'Grandstream', 'MST7MDT', 'MST7MDT'),
         ('grandstream', 'tz', 'Grandstream', 'PST8PDT', 'PST8PDT')
         ] )
      db.commit()
...
- Added support for an additional MAC vendor ID that Grandstream acquired 
