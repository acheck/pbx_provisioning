...
# check if MAC is a Grandstream device
if not (macaddr.startswith("000b82") or macaddr.startswith("c074ad")):
   syslog.syslog(syslog.LOG_ERR, "%s : This is not a Grandstream device  %s" % (str(sys.argv[0]), macaddr))
   sys.exit("This is not a Grandstream device")

dictfile = "/etc/asterisk/provisioning/gs_pval_dict.txt"
...
if os.path.isfile("/etc/asterisk/user_provisioning/gs_pval_dict.txt"):
   dictfile = "/etc/asterisk/user_provisioning/gs_pval_dict.txt"
...
file.write("<gs_provision version=\"1\">\n")
...
try:
   with open(tempfile) as conffile:
	 ...
	 #print "    <%s>%s</%s>" % (gsdict[x.strip()],y.strip(),gsdict[x.strip()])
	    ...
	    file.write("    <%s>%s</%s>\n" % (gsdict[x.strip()],y.strip(),gsdict[x.strip()]))
         else:
